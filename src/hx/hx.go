...
func (h *hx_tui) warn(msg string) {
	fmt.Fprintf(stderr_writer, "warning: %s\n", msg)
}
...
func (s hx_src) items_from_http(src_url *url.URL, yield func(hx_item) bool) error {
	...
	if s.download_only {
		...
		if insecure_retry {
			fmt.Fprintln(stderr_writer, tls_retry_message)
		}
...
func warn_windows_symlink_privilege_once() {
	windows_symlink_privilege_warning.Do(func() {
		fmt.Fprintln(stderr_writer, "warning: windows symlink privilege is unavailable, skipping symlink entries")
	})
}
...
func Main(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	fs.IntVar(&dst.skip_path_prefix, "delpathseg", 0, "drop N leading path segments from extracted entries")
	fs.StringVar(&src.platform, "platform", runtime.GOOS+"/"+runtime.GOARCH, "target platform")
	fs.StringVar(&src.platform, "plat", runtime.GOOS+"/"+runtime.GOARCH, "target platform")
	fs.StringVar(&src.registry_base_url, "registry", "", "registry override")
	fs.StringVar(&src.registry_base_url, "reg", "", "registry override")
	fs.StringVar(&src.target, "target", "", "target override")
	fs.StringVar(&src.target, "t", "", "target override")
	fs.StringVar(&dst.include_exclude, "incexc", ":+", "include/exclude rules")
	fs.StringVar(&dst.repath, "repath", "", "keep only matching destination suffixes after include/exclude filtering")
	fs.Var(bool_flag{&keep_symlinks}, "symlinks", "keep symlinks when supported")
	fs.Var(bool_flag{&src.download_only}, "download-only", "download without extraction")
	...
	if len(parsed_args) == 2 {
		dst.path = parsed_args[1]
	}

	if err := dst.copy(); err != nil {
		fmt.Fprintf(stderr_writer, "error: %v\n", err)
		return 1
	}
