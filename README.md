...
After a successful extraction/download `hx` writes a sentinel file in the destination. On subsequent runs with the same source, destination, `-strip`, `-symlinks`, `-download-only`, Docker `-platform`, and `-incexc` values it prints `already extracted, skipping` and exits 0 immediately.
...
- Changing `-registry`, `-target`, or `-incexc` also triggers a fresh extraction/download when they affect the materialized output.
